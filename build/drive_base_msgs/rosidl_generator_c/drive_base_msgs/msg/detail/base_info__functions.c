  ...
  drive_base_msgs__msg__BaseInfo__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    drive_base_msgs__msg__BaseInfo * data =
      (drive_base_msgs__msg__BaseInfo *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!drive_base_msgs__msg__BaseInfo__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          drive_base_msgs__msg__BaseInfo__fini(&data[i]);
        }
        free(data);
        return false;
      }
