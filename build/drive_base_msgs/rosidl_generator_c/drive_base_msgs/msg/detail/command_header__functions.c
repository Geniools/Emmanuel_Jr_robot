  ...
  drive_base_msgs__msg__CommandHeader__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    drive_base_msgs__msg__CommandHeader * data =
      (drive_base_msgs__msg__CommandHeader *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!drive_base_msgs__msg__CommandHeader__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          drive_base_msgs__msg__CommandHeader__fini(&data[i]);
        }
        free(data);
        return false;
      }
